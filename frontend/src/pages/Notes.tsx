...
const Notes: React.FC = () => {
  ...
  useEffect(() => {
    if (notes.length > 0 && noteOrder.length === 0) {
      setNoteOrder(notes.map((note) => note.id));
    }
  }, [notes, noteOrder.length]);
  ...
  const sortedNotes = useMemo(() => {
    const orderedNotes = noteOrder.length > 0 ? (noteOrder.map((id) => notes.find((note) => note.id === id)).filter(Boolean) as Note[]) : notes;
