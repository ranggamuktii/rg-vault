    ...
    <MainLayout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8 space-y-6 sm:space-y-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text">🔗 Links</h1>
            <p className="text-gray-600 mt-1 sm:mt-2 text-base sm:text-lg">Save and organize your bookmarks</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="block w-full pl-12 sm:pl-14 pr-4 sm:pr-5 py-3 sm:py-4 border border-gray-200 rounded-xl sm:rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white/80 backdrop-blur-sm shadow-sm text-sm sm:text-base"
              />
            </div>
            <div className="w-full sm:w-48">
              <select
                ...
                className="block w-full px-4 sm:px-5 py-3 sm:py-4 border border-gray-200 rounded-xl sm:rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white/80 backdrop-blur-sm shadow-sm text-sm sm:text-base"
              >
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12 sm:py-16" role="status" aria-label="Loading links">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              <div
                ...
                className="
                  bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/40 rounded-2xl sm:rounded-3xl border-2 border-dashed border-gray-200 
                  flex flex-col items-center justify-center p-6 sm:p-8 text-gray-500
                ...
                "
              >
                <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 mb-3 sm:mb-4 shadow-lg">
                  <PlusIcon className="h-6 sm:h-8 w-6 sm:w-8 text-white" />
                </div>
                <span className="font-semibold text-base sm:text-lg">Add Link</span>
              </div>
              ...
              {links.map((link, idx) => (
                <div
                  ...
                  className="
                    bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-5 sm:p-6 border border-gray-100 hover:border-gray-200
                ...
                >
                  <div className="flex items-start justify-between mb-4 sm:mb-5">
                    <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                      {link.favicon_url && (
                        <div className="w-6 sm:w-8 h-6 sm:h-8 rounded-lg sm:rounded-xl overflow-hidden shadow-sm bg-white flex-shrink-0">
                          <img
                      ...
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 truncate">{link.title || ''}</h3>
                    </div>
                    <div className="flex space-x-1 sm:space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <button
                        onClick={() => openLink(link.url)}
                        aria-label="Open link"
                        title="Open link"
                        className="p-2 sm:p-2.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg sm:rounded-xl transition-all duration-200 hover:scale-110"
                      >
                        <ArrowTopRightOnSquareIcon className="h-3.5 sm:h-4 w-3.5 sm:w-4" />
                      </button>
                      <button
                        onClick={() => openModal(link)}
                        aria-label="Edit link"
                        title="Edit link"
                        className="p-2 sm:p-2.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg sm:rounded-xl transition-all duration-200 hover:scale-110"
                      >
                      ...
                      <button
                        ...
                        title="Delete link"
                        className="p-2 sm:p-2.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg sm:rounded-xl transition-all duration-200 hover:scale-110"
                      >
                  ...
                  {link.description && <p className="text-gray-600 text-sm mb-4 sm:mb-5 line-clamp-3 leading-relaxed">{link.description}</p>}

                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <p className="text-blue-600 text-sm truncate flex-1 mr-2 sm:mr-3 font-semibold">{new URL(link.url).hostname}</p>
                    {link.category && <span className="inline-block bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 text-xs px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full font-semibold">{link.category}</span>}
                  </div>

                  <div className="text-xs text-gray-500 pt-3 sm:pt-4 border-t border-gray-100 font-medium">
